...
export class Buscador {
    ...
    async main(){
        ...
        const paginaInicial : Pagina = this.indexador.paginasBaixadas[0];
        this.calcularPontuacoes(paginaInicial,searched_term);
    }

    private calcularPontuacoes(pagina : Pagina, searched_term : string)  {// : ScoreObject
    ...
    private calcularAutoridade(paginas : Pagina[]){
        const nomesDasPaginas : string [] = [];
        paginas.forEach((pagina)=>nomesDasPaginas.push(pagina.getLink))
    }
