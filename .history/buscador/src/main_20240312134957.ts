...
function calcularUsoDeTags(html : string, searched_term : string, scores : ScoreObject){
    ...
    for(let h1 of h1s){
        if(h1.text.toU().includes(searched_term.toU())){
            h1s_ocorrencias += contarOcorrenciasSubstring(h1.text.toLocaleUpperCase());

        }
    }
    for(let h2 of h2s){
        if(h2.text.toU().includes(searched_term.toU()))
            h2s_ocorrencias++;
    }
    for(let p of ps){
        if(p.text.toU().includes(searched_term.toU())){
            ps_ocorrencias++;
            console.log("ACHEI P")
        }
    }
    for(let a of as){
        if(a.text.toU().includes(searched_term.toU()))
            as_ocorrencias++;
    }
